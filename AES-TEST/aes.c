...
{   
    ...
    else ciphertext = (PBYTE)calloc(ciphertextLength, sizeof(BYTE));        // Free needed
...
void AES_Dec_TEST(BCRYPT_ALG_HANDLE ALG_HANDLE)
{
    ...
    status = BCryptDecrypt(                         // Calculate decrypt text length
                           KEY_HANDLE,              // KEY HANDLE 
                           ...
                           IV,                      // Address of the buffer that contains the Initial Vector 
                           IVLength,                // Size of the buffer that contains the Initial Vector
                           NULL,                    // Address of the buffer that receives the decrypt text. 
                           0,                       // Size of the buffer that receives the decrypt text 
                           &decryptLength,          // Variable that receives number of bytes copied to decrypt text buffer
                           BCRYPT_BLOCK_PADDING);   // Flags : Block Padding 
    if(!NT_SUCCESS(status))
    {
        printf("Error Code : %x \n BCryptEncrypt(Calculate ciphertextLength) fail\n");
    ...
    else decrypt = (PBYTE)calloc(decryptLength, sizeof(BYTE));        // Free needed
    ...
    status = BCryptDecrypt(                         // Decrypt Data
                           ...
                           IV,                      // Address of the buffer that contains the Initial Vector 
                           IVLength,                // Size of the buffer that contains the Initial Vector
                           decrypt,                 // Address of the buffer that receives the decrypt text. 
                           decryptLength,           // Size of the buffer that receives the decrypt text 
                           &bufferSize,             // Variable that receives number of bytes copied to decrypt text buffer
                           BCRYPT_BLOCK_PADDING);   // Flags : Block Padding 
    if(!NT_SUCCESS(status))
    {
        printf("Error Code : %x \n BCryptEncrypt(Encrypt Data) fail\n");
    ...
    PRINT(decrypt, bufferSize, PRINT_DECRYPT);
...
{   
    ...
    ------------------------------------------------    
    [Encrypting Data]
        1. Get Cipher Algorithm Handle 
        2. Generating Symmetric Key
           - Generate 128 or 192 or 256 bit key      
        3. Setting Cipher Algorithm Properties
           - Set Block Chain Mode 
        4. Getting Cipher Algorithm Properties 
           - Get Block/IV Length
        5. Encrypting Data  
